...

        {
          clinics?.map((item) => (
            <div className='md:w-86 w-full shadow  bg-white rounded-2xl  my-4 '>
              <img src={item.userId.profileimg} className='w-full h-55 object-cover' alt="" />
              <div className='p-4'>

                <h2 className='text-xl body-font my-2 text-shadow-2xs'>{item.userId.name}</h2>
                <div>
                  ...
                  <p className="text-gray-600 text-sm leading-5">
                    <span className='mx-1'>Place - {item.place}</span>,
                    <span className='mx-1'>City - {item.city}</span>,
                    <span className='mx-1'>District - {item.dist}</span>,
                    <span className='mx-1'>State - {item.state}</span>
                    <span className='mx-1'>Pincode - {item.pincode}</span>
                  </p>
                </div>

                <div className='flex justify-center mt-3'>

                  <Link to={`/viewclinics/${item._id}`} className='text-xs text-shadow-2xs 
               bg-gradient-to-r from-[#035594] to-[#06AED4]  text-white p-1.5 rounded-3xl px-4 
                cursor-pointer'>View Clinic Page</Link>
                </div>
              </div>
            </div>
          ))
